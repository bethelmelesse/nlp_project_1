...
def extract_features_word_embedding(tweet):
    try:
        ...
        last_hidden_states = torch.mean(outputs.last_hidden_state, dim=-2).detach().cpu().numpy()
        tweet_vector = last_hidden_states[0, :]
    except:
        tweet_vector = np.zeros(200)
    return tweet_vector

def convert_to_tensors_word_embedding(processed_x, y, device):
    # extracting features 
    x = []
    for i, tweet in enumerate(processed_x):
        print(i)
...
class LinearRegressionModel(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_relu_stack = torch.nn.Sequential(
            torch.nn.Linear(256, 50, bias=True),
            torch.nn.ReLU(),
